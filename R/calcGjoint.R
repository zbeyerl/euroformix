...
calcGjoint = function(freq,nU=1,fst=0,refK=NULL,refR=NULL,ibd=c(1,0,0)) {
  ...
  P = function(Pi,ni,n) (ni*fst + (1-fst)*Pi)/(1+(n-1)*fst) #helpfunction for allele prob
    genoProb = function(mkvec2,refR=NULL,ibd2=NULL) { #helpfunction to get genotype prob for spec
      ...
      if(length(refR)==0 || is.null(ibd2)) return(Gprob0) #return if no relatedness
      
      #CONTINUE ASSUMING KINSHIP FOR UNKNOWN:
      if(length(refR)==1) {
        refR = rep(refR,2) #impute second allele
        warning("The related reference profile had only 1 allele in a marker. Assuming homozygous.")  #Assuming 2 alleles for refR
      }
